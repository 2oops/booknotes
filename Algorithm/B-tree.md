...
1. 位运算
   ...
   左移：其可以看成公式`a * (2 ^ b)`，比如说十进制的10，二进制是1010，左移一位，即是10100，即为20，也就是`10 * (2 ^ 1)`

   右移：公式可以看成是 `int v = a / (2 ^ b)`，比如十进制的10，右移一位变成101，即为十进制的5。

2. 冒泡排序：从第一个元素开始，将其与下一个元素逐个比较，从左到右挑大（高）的往右站，时间复杂度`O(n*n)`。

3. 插入排序：元素位置交换，时间复杂度`O(n*n)`

4. 选择排序：替换最小值索引，`O(n*n)`

5. 归并排序：可递归实现，`O(n * logN)`

6. 快速排序：比较基准值，交换位置。`O(logN)`

7. 堆排序：

8. 动态规划：大事化小，将一个大问题拆分成子问题，（考虑好边界问题）每次解决那些相似的子问题从而达到精简计算量的目的。一旦子问题得出解，存储下来备用。

   A：斐波那契数列的求解使用动态规划来优化解法
   ...
   function fib(n) {
       ...
       arr[1] = 1
       for(let i = 0; i <= n; i++) {
           arr[i] = arr[i - 1] + arr[i - 2]//通过数组存储数列的值
       }
       return arr[n]
   }
   ```
